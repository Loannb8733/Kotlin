...
class ActivityPhotoPartageFoot : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_photo_partage_foot)

        val intent = intent
        var n = 0
        if (intent.hasExtra("edittext")){ // vérifie qu'une valeur est associée à la clé “edittext”
            n = intent.getIntExtra("edittext", 0); // on récupère la valeur associée à la clé
        }
        val nbPoint = intent.getIntExtra("edittext", 0)
        val textView = findViewById<TextView>(R.id.scoreFinalFoot)
        ...
        val backHome = findViewById<ImageView>(R.id.retourAccueil2)
        ...
        when (requestCode) {
            REQUEST_CODE -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    val intent = Intent(this@ActivityPhotoPartageFoot, ActivityPhotoPartageFoot::class.java)
                    startActivity(intent)
                } else {
                    askForPermissions()
                }
