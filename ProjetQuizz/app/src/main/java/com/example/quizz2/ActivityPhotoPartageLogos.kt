...
class ActivityPhotoPartageLogos : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_photo_partage_logos)

        val intent = intent
        var n = 0
        if (intent.hasExtra("nombreFinalPointLogos")){ // vérifie qu'une valeur est associée à la clé “edittext”
            n = intent.getIntExtra("nombreFinalPointLogos", 0); // on récupère la valeur associée à la clé
        }
        val nbPoint = intent.getIntExtra("nombreFinalPointLogos", 0)
        ...
        val backHome = findViewById<ImageView>(R.id.retourAccueil7)
        ...
        when (requestCode) {
            REQUEST_CODE -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    val intent = Intent(this@ActivityPhotoPartageLogos, ActivityPhotoPartageLogos::class.java)
                    startActivity(intent)
                } else {
                    askForPermissions()
                }
