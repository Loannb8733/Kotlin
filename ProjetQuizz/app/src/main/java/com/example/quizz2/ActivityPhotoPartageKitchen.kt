...
class ActivityPhotoPartageKitchen : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_photo_partage_kitchen)

        val intent = intent
        var n = 0
        if (intent.hasExtra("nombreFinalPointKitchen")){ // vérifie qu'une valeur est associée à la clé “edittext”
            n = intent.getIntExtra("nombreFinalPointKitchen", 0); // on récupère la valeur associée à la clé
        }
        val nbPoint = intent.getIntExtra("nombreFinalPointKitchen", 0)
        val textView = findViewById<TextView>(R.id.scoreFinalLogos)
        ...
        val backHome = findViewById<ImageView>(R.id.retourAccueil6)
        ...
        when (requestCode) {
            REQUEST_CODE -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    val intent = Intent(this@ActivityPhotoPartageKitchen, ActivityPhotoPartageKitchen::class.java)
                    startActivity(intent)
                } else {
                    askForPermissions()
                }
