...
class ActivityPhotoPartageGeneralCulture : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_photo_partage_general_culture)

        val intent = intent
        var n = 0
        if (intent.hasExtra("nombreFinalPointGeneralCulture")){ // vérifie qu'une valeur est associée à la clé “edittext”
            n = intent.getIntExtra("nombreFinalPointGeneralCulture", 0); // on récupère la valeur associée à la clé
        }
        val nbPoint = intent.getIntExtra("nombreFinalPointGeneralCulture", 0)
        val textView = findViewById<TextView>(R.id.scoreFinalGeneralCulture)
        ...
        val backHome = findViewById<ImageView>(R.id.retourAccueil4)
        ...
        when (requestCode) {
            REQUEST_CODE -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    val intent = Intent(this@ActivityPhotoPartageGeneralCulture, ActivityPhotoPartageGeneralCulture::class.java)
                    startActivity(intent)
                } else {
                    askForPermissions()
                }
